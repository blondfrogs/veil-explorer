    ...
    : ControllerBase
{
    ...
    public async Task<IActionResult> Get(TxRequest body, CancellationToken cancellationToken)
    {
        ...
        {
            return Problem("Failed to retrieve transaction details. The transaction may not be available or there was an error communicating with the node.", statusCode: 500);
        }
    }
}
